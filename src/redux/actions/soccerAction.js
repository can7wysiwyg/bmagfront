  ...
  return async function (dispatch) {
    try {
      const response = await axios.post(
        `${ApiUrl}/admin_soccer_table_create`,
        teams,
        {
          headers: {
...
export function getTables() {
  return async function (dispatch) {
    try {
      const response = await axios.get(`${ApiUrl}/tables_all`);
      const tables = response.data.tables;

      dispatch({ type: GET_TABLES, payload: tables });
    } catch (error) {
      console.log("there was a problem " + error);
      dispatch({ type: SOCCER_ERROR });
      throw error;
  ...
  return async function (dispatch) {
    try {
      const response = await axios.get(
        `${ApiUrl}/results_by_league_name/${id}`
      );
...
export function getGamesByLeague(id) {
  return async function (dispatch) {
    try {
      const response = await axios.get(
        `${ApiUrl}/public_show_games_by_league/${id}`
      );
...
export function updateTeamStats(teams, id) {
  return async function (dispatch) {
    try {
      const response = await axios.put(`${ApiUrl}/update_table/${id}`, teams, {
        headers: {
          Authorization: `Bearer ${bmagtoken}`,
        },
      });
